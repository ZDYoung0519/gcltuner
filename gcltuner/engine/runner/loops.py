...
class ValLoop(MMENGINE_ValLoop):
    def __init__(self, runner, dataloader, evaluator=None, torch_dtype='fp16', select_metric='avg') -> None:
        # must be concatset
    ...
    def run(self) -> dict:
        ...
        for task_id, dataset in enumerate(self.dataloader.dataset.datasets):
            model.preparing_for_generation(dataset.tokenizer, dataset.meta_info)
            ...
            results.sort(key=lambda e: e['question_id'])

            self.runner.logger.info('========= Starting the evaluation of a data ===========')
            if is_main_process():
                work_dir = os.path.join(self.runner.work_dir, f"task{task_id}")
...
class TestLoop(ValLoop):
    def run(self) -> dict:
        ...
        for task_id, dataset in enumerate(self.dataloader.dataset.datasets):
            ...
            print(f"Dataset: {dataset.meta_info['name']}, Metric: {metric}")

        # select metrics
        if self.select_metric == 'first':
            metrics = metrics[0]
        elif self.select_metric == 'avg':
            avg_metric = sum([sum(m.values()) for m in metrics]) / len(metric)
            metrics = {"avg": avg_metric}
        else:
            raise NotImplementedError
    ...
    def run_iter(self, idx, data_batch: Sequence[dict], results: list):
        ...
        Args:
            data_batch (Sequence[dict]): Batch of data
                from dataloader.
        """
        assert 'index' in data_batch['data'], 'index is required in data_batch. ' \
                                               'The __getitem__ function in the dataset must ' \
                                               'return a dictionary with the index.'

        index = data_batch['data'].pop('index', [None])[0]
