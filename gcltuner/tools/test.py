...
def parse_args():
    ...
    parser.add_argument("--eval-only-cur-task", action='store_true')
    parser.add_argument(
        "--work-dir",
...
def main():
    ...
    # cfg.test_dataloader['dataset']['datasets'] = cfg.test_dataset[args.cur_task:(args.cur_task+1)]
    if not args.eval_only_cur_task:
        cfg.test_dataloader['dataset']['datasets'] = cfg.test_dataset[:(args.cur_task+1)]
    else:
        cfg.test_dataloader['dataset']['datasets'] = [cfg.test_dataset[args.cur_task]]
    cfg.visualizer = None
    ...
    if is_model_wrapper(runner.model):
        runner.model.module.load_checkpoint(args.checkpoint)
    else:
        runner.model.load_checkpoint(args.checkpoint)
