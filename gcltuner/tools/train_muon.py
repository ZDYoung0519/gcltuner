...
from xtuner.tools.utils import (
...
def parse_args():
    ...
    parser.add_argument("--muon_lr", default=0.02, type=float, help='task identity')
    parser.add_argument("--adam_lr", default=2e-4, type=float, help='task identity')
    parser.add_argument("--muon_momentum", default=0.99, type=float, help='task identity')
    parser.add_argument("--adam_w_mode", action='store_true', help='task identity')
    parser.add_argument(
        "--deepspeed",
        type=str,
        default=None,
...
def check_cfg(cfg, args):
    ...
    if getattr(cfg, "sequence_parallel_size", 1) > 1:
        assert SUPPORT_FLASH2, (
            "`flash_attn` is required if you want to use " "sequence parallel."
        )
    ...
    if getattr(cfg, "use_varlen_attn", False):
        assert SUPPORT_FLASH2, (
            "`flash_attn` is required if you set " "`use_varlen_attn` to True."
        )
        attn_implementation = getattr(cfg.model.llm, "attn_implementation", None)
        assert (
            attn_implementation is None or attn_implementation == "flash_attention_2"
    ...
    if cfg.get("framework", "mmengine").lower() == "huggingface":
        ...
        if args.deepspeed:
            ...
            if digit_version(deepspeed.__version__) < digit_version("0.12.3"):
                raise RuntimeError(
                    "Please upgrade your DeepSpeed version "
            ...
            if optim_wrapper == "DeepSpeedOptimWrapper":
                ...
                strategy = dict(
                    type=LazyObject("gcltuner.engine", "DeepSpeedStrategyWithMuon"),
                    config=ds_cfg,
                ...
                muon_lr = cfg.get("muon_lr", args.muon_lr)
                adam_lr = cfg.get("adam_lr", args.adam_lr)
                ...
                ds_cfg['optimizer'] = optimizer

                cfg.__setitem__("strategy", strategy)
                optim_wrapper = dict(
                    type="DeepSpeedOptimWrapper", optimizer=cfg.optim_wrapper.optimizer
                )
                
                # modify here, add paramwise_cfg when using deepspeed
