...
class CoINMOELoraConfig(LoraConfig):
    ...
    def __post_init__(self):
        self.peft_type = PeftType.COINMOELORA
...
class CoINMOELoraModel(LoraModel):
    ...
    def _create_new_module(self, lora_config, adapter_name, target):
        ...
        if loaded_in_8bit and isinstance(target, bnb.nn.Linear8bitLt):
            ...
            new_module = CoINMOELLoraLinear8bitLt(
            ...
            new_module = CoINMOELoraLinear4bit(adapter_name, target.in_features, target.out_features, bias=bias, **fourbit_kwargs)
...
if is_bnb_available():
    class CoINMOELLoraLinear8bitLt(bnb.nn.Linear8bitLt, CoINMOELoraLayer):
        def __init__(
            self,
            adapter_name,
            in_features,
    ...
    if is_bnb_4bit_available():
        class CoINMOELoraLinear4bit(bnb.nn.Linear4bit, CoINMOELoraLayer):
            # Lora implemented in a dense layer
            def __init__(
                self,
                adapter_name,
