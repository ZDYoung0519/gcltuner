...
class DiscoMoeLoraConfig(LoraConfig):
    """
    This is the configuration class to store the configuration of a [`~peft.DISCO_MOE_LORA`]
    """
    ...
    def __post_init__(self):
        self.peft_type = PeftType.DISCOMOELORA
...
class CoINMOELoraLayer(LoraLayer):
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights):
        ...
        if r > 0:
            self.lora_A.update(nn.ModuleDict({adapter_name: CoINMOELinearA(self.in_features, r, self.expert_num, self.cur_task, self.training)}))
            self.lora_B.update(nn.ModuleDict({adapter_name: CoINMOELinearB(r, self.out_features, self.expert_num, self.cur_task, self.training)}))
...
class CoINMOELoraLinear(nn.Linear, CoINMOELoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.mask_signal = [1.0, 0, 0, 0, 0, 0, 0, 0]
            ...
            for i, flag in enumerate(self.mask_signal):
                if flag != 0:
                    ...
                    if end > start and self.diag_size > 0:
                        self.lora_AB[self.active_adapter].weight.data[start:end, start:end] = torch.eye(self.diag_size) * flag
            self.lora_AB = self.lora_AB.to(x.device)

            if self.training:
                lora_a_output = self.lora_A[self.active_adapter].loraA[self.lora_id](self.lora_dropout[self.active_adapter](x))
                ...
                lora_ab_output = self.lora_AB[self.active_adapter](lora_a_output)
                lora_b_output = self.lora_B[self.active_adapter](lora_ab_output)
                result += lora_b_output * self.scaling[self.active_adapter]
        else:
            result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
...
class CoINMOELinearA(nn.Module):
    ...
    def __init__(self, in_features, out_features, expert_num, cur_task, training) -> None:
    ...
    def forward(self, x):
        ...
        if self.training:
            assert 0 <= self.lora_id < self.expert_num, "Invalid current_task value"
            output = self.loraA[self.lora_id](x)
            return output
        else:
            total_out_features = self.expert_num  * self.r
            temp_mlp = nn.Linear(self.in_features, total_out_features, bias=False).to(x.device)
            concatenated_weight = []
            
            for i in range(self.expert_num):
                concatenated_weight.append(self.loraA[i].mlp.weight)

            concatenated_weight = torch.cat(concatenated_weight, dim=0)
...
class CoINMOELinearB(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            assert 0 <= self.lora_id < self.expert_num, "Invalid current_task value"
            output = self.loraB[self.lora_id](x)
            return output
        else:
            total_in_features = (self.expert_num) * self.r
            temp_mlp = nn.Linear(total_in_features, self.out_features, bias=False).to(x.device)
            concatenated_weight = []
            
            for i in range(self.expert_num):
                concatenated_weight.append(self.loraB[i].mlp.weight)

            concatenated_weight = torch.cat(concatenated_weight, dim=1)
...
class CoINMOERouter(nn.Module):
    ...
    def __init__(self, config: DiscoMoeLoraConfig):
        super().__init__()
