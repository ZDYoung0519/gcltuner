...
# # We need to merge the current params with the previous ones
# if [ "$TASKID" -gt 0 ]; then
#     # combine weights
#     echo "merging trained checkpoints...."
#     python projects/aligncl/tools/merge_checkpoints.py --cur-ckpt $CKPT --pre-ckpt $LASTCKPT --output $WORKDIR/task$TASKID/final.pth
#     # write path to the file, so we can read it correctly during the next task
