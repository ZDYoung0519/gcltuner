...
def main():
    ...
    cfg.visualizer = None
    cfg.model = dict(
        type="projects.aligncl.model.toy_model.ToyModel",
        visual_encoder=cfg.model['visual_encoder'],
        text_encoder=cfg.model['text_encoder'],
        text_tokenizer=cfg.model['text_tokenizer'],
        cur_task=cfg.model['cur_task'],
        expert_num=cfg.model['expert_num'],
        expert_router_args=cfg.model['expert_router_args'],
    ...
    gaussian_memory = GaussianMemory(1, args.cov_mode, args.l2_norm)
    ...
    def train_epoch(cur_task):
        for j in range(cur_task+1):
            means = gaussian_memory.means[j][0]
            # print(means)
            model.expert_router.weight.data[j].copy_(means)
        return model
    ...
    def validate_until_now(model, cur_task):
        ...
        for task_id in range(cur_task+1):
            ...
            logits = model.expert_router(task_features)
    ...
    for epoch in epoch_pbar:
        ...
        if is_main_process():
            ...
            if val_acc1 > best_acc:
                ...
                state_dict = model.expert_router.state_dict()
