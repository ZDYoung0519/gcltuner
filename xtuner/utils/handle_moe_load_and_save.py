...
def _merge_experts_weight(state_dict, expert_num_per_shard, order_mapping):
    ...
    for shard_idx in range(expert_shard_num):
        begin, end = (
            shard_idx * linear_num_per_shard,
...
def load_state_dict_into_model(model_to_load, pretrained_model_path):
    ...
    safe_index_file = os.path.join(pretrained_model_path, SAFE_WEIGHTS_INDEX_NAME)
    ...
    def load(module: nn.Module, state_dict, unloaded_shard_files, prefix=""):
        params_to_gather = []
        param_names = []
        for name, param in module.named_parameters(prefix=prefix[:-1], recurse=False):
            while name not in state_dict:
                ...
                _merge_experts_weight(state_dict, expert_num_per_shard, order_mapping)
        ...
        for name, child in module._modules.items():
            if child is not None:
                load(child, state_dict, unloaded_shard_files, prefix + name + ".")
...
def _get_origin_param_name(
    merged_param_name, expert_num_per_shard, is_w1w3, param_name_mapping
):
    ...
    for i in range(expert_num_per_shard):
        if is_w1w3:
            gate_proj, up_proj = (
                param_name_mapping["gate_proj"],
                param_name_mapping["up_proj"],
            )
            gate = split_name[0] + f".experts.{expert_idx_begin + i}.{gate_proj}.weight"
            up = split_name[0] + f".experts.{expert_idx_begin + i}.{up_proj}.weight"
            origin_param_names[i * 2] = gate
            origin_param_names[i * 2 + 1] = up
        else:
            down_proj = param_name_mapping["down_proj"]
            down = split_name[0] + f".experts.{expert_idx_begin + i}.{down_proj}.weight"
...
def get_origin_state_dict(state_dict, model):
    ...
    experts_param_name = [name for name in state_dict.keys() if ".experts." in name]
    for expert_param_name in experts_param_name:
        ...
        origin_param_names = _get_origin_param_name(
            expert_param_name, expert_num_per_shard, is_w1w3, param_name_mapping
        )
