# From https://github.com/microsoft/dion
import torch
import triton
...
def _pid_to_block(
    ...
    """Helper function to map Triton program ID to (batch, row, col) of the
    output matrix."""
...
def ns_line_1_kernel(
    ...
    Output C has shape (M, M) Compute C = A @ A.T
    """
...
def ns_line_2_kernel(
    ...
    Output C has shape (M, M) Compute C = alpha * A @ A.T + beta * A
    """
...
def ns_line_2(A: Tensor, alpha: float, beta: float, *, out: Tensor | None = None):
    ...
    M, K = A.shape[-2:]
    if M != K:
        raise ValueError(f"Input must be symmetric square matrix, but got shape {A.shape}")
