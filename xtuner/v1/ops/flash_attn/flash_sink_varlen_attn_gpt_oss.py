...
def _bwd_preprocess_do_o_dot(
    ...
    do = tl.load(
        DO + off_h * stride_doh + offs_m[:, None] * stride_dom + offs_d[None, :],
        mask=mask_m[:, None],
        other=0.0,
    ).to(tl.float32)
...
class FlashSinkVarlenAttention(torch.autograd.Function):
    ...
    def forward(
        ...
        o, lse, ctx.softmax_scale = _flash_attn_forward(
            q, k, v, sink=sink, cu_seqlen=cu_seqlen, sliding=window_size, softmax_scale=None
        )
    ...
    def backward(ctx, do, dlse):
        ...
        _flash_attn_backward(
            o, do, q, k, v, dq, dk, dv, ctx.sink, dsink, ctx.cu_seqlen, ctx.window_size, lse, ctx.softmax_scale
        )
