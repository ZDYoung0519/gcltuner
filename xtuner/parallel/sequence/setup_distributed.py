...
def init_inner_sequence_parallel(inner_sequence_parallel_size: int = 1):
    ...
    They are helpful when sp size is not evenly divided by the number of attn heads.
    """
    ...
    for i in range(n_inner_group):
        ranks = range(
            i * inner_sequence_parallel_size, (i + 1) * inner_sequence_parallel_size
        )
        group = dist.new_group(ranks)
        if rank in ranks:
...
def get_sequence_parallel_rank():
    ...
    if not dist.is_initialized() or (_SEQUENCE_PARALLEL_GROUP is None):
        ...
        _SEQUENCE_PARALLEL_RANK = dist.get_rank(group=get_sequence_parallel_group())
    return _SEQUENCE_PARALLEL_RANK
...
def get_data_parallel_world_size():
    ...
    if not dist.is_initialized():
        ...
        _DATA_PARALLEL_WORLD_SIZE = dist.get_world_size(group=get_data_parallel_group())
    return _DATA_PARALLEL_WORLD_SIZE
