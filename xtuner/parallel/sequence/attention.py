...
def pre_process_for_sequence_parallel_attn(
    ...
    def pre_process_for_inner_sp(q, k, v):
        ...
        q = (
            q.view(b, s_div_sp, sp // insp, h * insp // sp, insp, d // insp)
            .transpose(3, 4)
            .flatten(2, 4)
        )
        k = (
            k.view(b, s_div_sp, sp // insp, h * insp // sp, insp, d // insp)
            .transpose(3, 4)
            .flatten(2, 4)
        )
        v = (
            v.view(b, s_div_sp, sp // insp, h * insp // sp, insp, d // insp)
            .transpose(3, 4)
            .flatten(2, 4)
        )
    ...
    def post_process_for_inner_sp(q, k, v):
        ...
        q = gather_forward_split_backward(q, -1, get_inner_sequence_parallel_group())
        k = gather_forward_split_backward(k, -1, get_inner_sequence_parallel_group())
        v = gather_forward_split_backward(v, -1, get_inner_sequence_parallel_group())
...
def post_process_for_sequence_parallel_attn(attn_output, scatter_dim=1, gather_dim=2):
    sp = get_sequence_parallel_world_size()
    insp = get_inner_sequence_parallel_world_size()
    ...
    if insp > 1:
        ...
        # (b, s_div_sp, h, d)
        output = (
            output.view(b, s_div_sp, sp // insp, insp, h * insp // sp, d // insp)
            .transpose(3, 4)
            .reshape(b, s_div_sp, h, d)
        )
...
def sequence_parallel_wrapper(local_attn):
    def sequence_parallel_attn(query_states, key_states, value_states, *args, **kwargs):
        ...
        out = local_attn(query_states, key_states, value_states, *args, **kwargs)
