...
class ThroughputHook(Hook):
    ...
    def _guess_is_casual_attn(model):
        ...
        print_log(
            ...
            "and FLOPs will be calculated as `casual = True`.",
            "current",
        )
        return True
    ...
    def before_run(self, runner) -> None:
        ...
        self.use_activation_checkpointing = (
            self.use_activation_checkpointing
            or self._guess_use_activation_checkpointing(model)
        )
    ...
    def after_train_iter(
        ...
        flops_attn = (
            ...
            * sequence_parallel_size
            / (int(self.is_casual) + 1)
        )
        flops_mlp = (
            4 * self.mlp_ratio * batch_size * sequence_len * self.hidden_size**2
        )
        flops_wo_head = (
            (3 + int(self.use_activation_checkpointing))
            * (flops_qkvo_proj + flops_attn + flops_mlp)
            * self.num_layers
        )
        flops_head = (
            3 * 2 * batch_size * sequence_len * self.hidden_size * self.vocab_size
        )
        flops_per_iteration = flops_wo_head + flops_head

        avg_tflops_per_gpu = flops_per_iteration / 1e12 / (iter_time + 1e-12)
        tokens_per_sec_per_gpu = batch_size * sequence_len / (iter_time + 1e-12)

        message_hub.update_scalar("train/tflops", avg_tflops_per_gpu)
        message_hub.update_scalar("train/tokens_per_sec", tokens_per_sec_per_gpu)
