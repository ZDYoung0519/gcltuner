...
class RewardModel(BaseModel):
    def __init__(
        ...
        assert penalty_type in (
            ...
            "L2",
            "none",
        ), f"Unsupported penalty type {penalty_type}"
        ...
        if (
            ...
            or isinstance(lora, Config)
            or isinstance(lora, ConfigDict)
        ):
    ...
    def _prepare_for_lora(self, peft_model=None, use_activation_checkpointing=True):
    ...
    def _prepare_for_long_context_training(cfg, llm_cfg, max_position_embeddings):
        if not hasattr(llm_cfg, "rope_scaling"):
            print_log("Current model does not support RoPE scaling.", "current")
            return
    ...
    def _prepare_for_flash_attn(cfg, llm_cfg):
        ...
        torch_dtype = (
            torch.bfloat16
            if (
                get_torch_device().is_available()
                and get_torch_device().is_bf16_supported()
            )
            else torch.float16
        )
    ...
    def _prepare_for_qlora_zero3(cfg):
        if (not is_deepspeed_zero3_enabled()) or (
            not hasattr(cfg, "quantization_config")
        ):
            return cfg
    ...
    def compute_loss(self, data, labels=None):
        ...
        if get_sequence_parallel_world_size() > 1:
            logits = gather_forward_split_backward(
                logits, dim=1, sp_group=get_sequence_parallel_group(), grad_scale="up"
            )
        ...
        num_samples = torch.tensor(len(chosen_logits)).float().to(hidden_states.device)
        ...
        if self.penalty_type == "log_barrier":
            ...
            penalty = self.l2_penalty(
                torch.cat([chosen_logits, rejected_logits]), avg_factor=avg_factor
            )
    ...
    def log_barrier_penalty(
        self, logits, lower_bound, upper_bound, epsilon=1e-3, avg_factor=1
    ):
        # log barrier penalty from InternLM2 paper https://arxiv.org/abs/2403.17297  # noqa
        logits_fp32 = logits.float()
        logits_clamped = torch.clamp(
            logits_fp32, lower_bound + epsilon, upper_bound - epsilon
        )
    ...
    def to_hf(self, cfg, save_dir, fp32=False, save_pretrained_kwargs={}, **kwargs):
        ...
        if "InternLM2" in self.llm.__class__.__name__:
            from xtuner.tools.model_converters.modeling_internlm2_reward.modeling_internlm2 import (  # noqa
                InternLM2ForRewardModel,
            )

            print(f"Saving Reward Model to {save_dir}")
            hf_cfg = self.llm.config
            hf_cfg.reward_token_id = (
                self.reward_token_id
                if self.reward_token_id is not None
                else cfg.reward_token_id
            )
            if not fp32:
                dtype = torch.float16
            else:
            ...
            try:
                with no_init_weights():
                    reward_model = AutoModelForSequenceClassification.from_config(
                        hf_cfg
                    )
            except Exception as e:
                warnings.warn(
                    f"Cannot find SequenceClassification class "
                ...
                seqcls_model_name = (
                    causal_model_name.split("For")[0] + "ForSequenceClassification"
                )
