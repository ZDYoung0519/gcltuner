...
class SupervisedFinetune(BaseModel):
    def __init__(
        ...
        self.llm = self.build_llm_from_cfg(
            llm, use_varlen_attn, max_position_embeddings
        )
    ...
    def build_llm_from_cfg(self, llm_cfg, use_varlen_attn, max_position_embeddings):
        # For forward
        with LoadWoInit():
    ...
    def _compute_sequence_parallel_loss(self, data):
        ...
        sp_group = get_sequence_parallel_group()
        loss = reduce_sequence_parallel_loss(outputs.loss, num_tokens, sp_group)
        return {"loss": loss}
