...
def load_balancing_loss_func(gate_logits: torch.Tensor,
    ...
    if attention_mask is None:
        ...
        router_per_expert_attention_mask = (attention_mask[None, :, :, None].expand(
            (num_hidden_layers, batch_size, sequence_length, num_experts)).reshape(-1, num_experts).to(compute_device))
...
class MixtralFlashAttention2(MixtralAttention):
    ...
    This module inherits from `MixtralAttention` as the weights of the module stays untouched. The only required change
    would be on the forward pass where it needs to correctly call the public API of flash attention and deal with
    padding tokens in case the input contains any of them.
    """
    ...
    def forward(
        ...
        use_sliding_windows = (_flash_supports_window_size and getattr(self.config, 'sliding_window', None) is not None
    ...
    def _upad_input(self, query_layer, key_layer, value_layer, attention_mask, query_length):
        ...
        if query_length == kv_seq_len:
            query_layer = index_first_axis(query_layer.reshape(batch_size * kv_seq_len, num_heads, head_dim),
                                           indices_k)
...
class MixtralSdpaAttention(MixtralAttention):
    ...
    This module inherits from `MixtralAttention` as the weights of the module stays untouched. The only changes are on
    the forward pass to adapt to SDPA API.
    """
...
class MixtralSparseMoeBlock(nn.Module):
    ...
    It's faster since it formulates MoE operations in terms of block-sparse operations to accommodate imbalanced
    assignments of tokens to experts, whereas standard MoE either (1) drop tokens at the cost of reduced performance or
    (2) set capacity factor to number of experts and thus waste computation and memory on padding.
    """
...
class ExpertShard(nn.Module):
    ...
    def __init__(self, config, expert_in_one_shard=1):
        super().__init__()
        self.w1w3 = nn.Parameter(torch.empty(expert_in_one_shard, config.intermediate_size * 2, config.hidden_size))
        self.w2 = nn.Parameter(torch.empty(expert_in_one_shard, config.hidden_size, config.intermediate_size))
        self.act = ACT2FN[config.hidden_act]
        self.expert_in_one_shard = expert_in_one_shard

    def forward(self, hidden_states, expert_mask, routing_weights, final_hidden_states):
...
class MixtralSparseShardMoeBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.expert_in_one_shard = expert_in_one_shard
        self.experts = nn.ModuleList([ExpertShard(config, self.expert_in_one_shard) for i in range(self.shard_num)])
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        for shard_index in range(self.shard_num):
            mask = expert_mask[shard_index * self.expert_in_one_shard:(shard_index + 1) * self.expert_in_one_shard]
            final_hidden_states = self.experts[shard_index](hidden_states, mask, routing_weights, final_hidden_states)
...
def _load_pretrained_model(
    ...
    if ((state_dict is not None) or (resolved_archive_file is None) or (low_cpu_mem_usage) or (device_map is not None)
            or (offload_folder is not None) or (not (offload_state_dict is None or offload_state_dict is False))
            or (hf_quantizer is not None) or (keep_in_fp32_modules is not None and len(keep_in_fp32_modules) > 0)
...
class MixtralPreTrainedModel(PreTrainedModel):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *args, **kwargs):
        ...
        cls._load_pretrained_model = types.MethodType(_load_pretrained_model, cls)
        return super().from_pretrained(pretrained_model_name_or_path, *args, **kwargs)
...
class MixtralModel(MixtralPreTrainedModel):
    ...
    def forward(
        ...
        if not return_dict:
            return tuple(v for v in [hidden_states, next_cache, all_hidden_states, all_self_attns, all_router_logits]
                         if v is not None)
