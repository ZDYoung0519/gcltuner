...
class LLaVAModel(BaseModel):
    def __init__(
        ...
        if use_activation_checkpointing:
            ...
            if hasattr(self.visual_encoder, "enable_input_require_grads"):
                ...
                self.visual_encoder.get_input_embeddings().register_forward_hook(
                    make_inputs_require_grad
                )
    ...
    def _prepare_visual_encoder_for_lora(
        self, lora_config, use_activation_checkpointing=True
    ):
        lora_config = self._parse_lora_config(lora_config)
        if lora_config.target_modules is None:
    ...
    def to_hf(
        ...
        **kwargs,
        ...
        if save_format == "xtuner":
            ...
            self.to_huggingface_llava(cfg, save_dir, fp32, save_pretrained_kwargs)
        elif save_format == "official":
            self.to_official_llava(cfg, save_dir, fp32, save_pretrained_kwargs)
        else:
            raise NotImplementedError

    def to_xtuner_llava(self, cfg, save_dir, fp32=False, save_pretrained_kwargs={}):
        # LLM
        ...
        if self.use_visual_encoder_lora:
            ...
            print_log(
                f"Saving visual_encoder adapter to {visual_encoder_path}", "current"
            )
            self.visual_encoder.save_pretrained(
                visual_encoder_path, **save_pretrained_kwargs
            )
        elif not self.freeze_visual_encoder:
        ...
        self.projector.save_pretrained(projector_path, **save_pretrained_kwargs)

    def to_huggingface_llava(
        self, cfg, save_dir, fp32=False, save_pretrained_kwargs={}
    ):
        LLM_MAPPING = {
        ...
        assert (
            getattr(self.llm, "hf_quantizer", None) is None
        ...
        need_visual_encoder = (
            not self.freeze_visual_encoder or self.use_visual_encoder_lora
        )
        ...
        if added_token > 0:
            ...
            model.language_model.lm_head.weight.data[ori_vocab_size:] = torch.stack(
                tuple(
                    dist.sample()
                    for _ in range(
                        model.language_model.lm_head.weight.data[ori_vocab_size:].shape[
                            0
                        ]
                    )
                ),
        ...
        model.config.image_token_index = tokenizer.encode(DEFAULT_IMAGE_TOKEN)[-1]
