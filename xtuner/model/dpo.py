...
class DPO(SupervisedFinetune):
        ...
        **kwargs,
        ...
        if ref_llm is not None:
            ref_llm = self.build_llm_from_cfg(
                ref_llm,
                kwargs.get("use_varlen_attn", False),
    ...
    def get_var_len_atten_logps(
        self, policy_logps, ref_logps, loss_mask, cu_seqlens, attention_mask
    ):
        ...
        for i in range(len(unpacked_policy_logps) // 2):
            ...
            policy_rejected_logps.append(compute_logps(rejected, rejected_mask))
            reference_chosen_logps.append(compute_logps(chosen_ref, chosen_mask))
            reference_rejected_logps.append(compute_logps(rejected_ref, rejected_mask))
    ...
    def compute_loss(self, data, data_samples=None):
        ...
        all_loss_mask = (
            data["labels"] != -100
        )  # loss mask of all tokens in all sp ranks  # noqa
        ...
        policy_logps = self._gather_masked_logits(all_logits, labels, loss_mask)
        ref_logps = self._gather_masked_logits(all_ref_logits, labels, loss_mask)
        ...
        if not self.use_varlen_attn:
            (
                ...
                reference_chosen_logps,
                reference_rejected_logps,
            ) = self.get_logps(policy_logps, ref_logps, all_loss_mask)
        else:
