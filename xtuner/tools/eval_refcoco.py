...
def eval_iou(answers):
    ...
    for answer in answers:
        bbox = answer["bbox"]
        bbox = RefCOCOJsonEvalDataset.normalize_bbox(
            bbox, answer["height"], answer["width"]
        )
...
def generate(
    ...
    pixel_values = projector(visual_outputs.hidden_states[visual_select_layer][:, 1:])
...
def main():
    ...
    # build_model
    llm, visual_encoder, projector, tokenizer, image_processor = build_model(args)
