...
from xtuner.dataset.map_fns import DATASET_FORMAT_MAPPING, template_map_fn_factory
...
def parse_args():
    ...
    parser.add_argument("--max-length", default=32768, help="Max sequence length.")
    parser.add_argument(
        "--pack-to-max-length",
        action="store_true",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "The path to a txt file which contains the a list of data path."
...
def process_untokenized_dataset(
    folder,
    tokenizer,
    max_length,
    pack_to_max_length,
    ...
    template_map_fn = ConfigDict(type=template_map_fn_factory, template=prompt_template)
