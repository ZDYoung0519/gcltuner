...
def chatml_format(
    ...
    for dialog_item in _processed_data:
        ...
        if not role_cfg[role]["loss"].get("begin", False):
            begin_token = [-token_id for token_id in begin_token]
        end_token = tokenizer.encode(role_cfg[role]["end"], add_special_tokens=False)
        # breakpoint()
...
def write_bin_meta_bin(path, dataset_name, filename, samples):
    ...
    if valid_samples > 500:
        np.save(open(valid_dir.joinpath(f"{filename}.bin.meta"), "wb"), valid_meta)
    else:
        print(
            f"{valid_f_path} is removed because the number of",
...
def tokenize_and_save(tokenizer, processed_dir, tokenized_dir):
    ...
    for filename in list_dir_or_file(data_dir, recursive=True, list_dir=False):
        ...
        tokenize_fun = partial(
            chatml_format, tokenizer=tokenizer, **CHATML_LLAMAV13_32K_TOKEN_CFG
        )
        samples = []
        ...
        if train_tokens is None:
            print(
                f"{osp.splitext(osp.basename(filename))[0]} already " "exists, skip it"
            )
            continue
...
def parse_args():
    ...
    parser.add_argument("--processed-dir", help="The folder to save untokenized data.")
    parser.add_argument("--tokenized-dir", help="The folder to save tokenized data.")
    parser.add_argument("--tokenizer-path", help="The path to the hf tokenizer.")
    parser.add_argument(
