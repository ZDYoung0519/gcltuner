...
class DecodeOutputStreamer(BaseStreamer):
    ...
    def __init__(self, tokenizer, skip_prompt=True) -> None:
        ...
        # TODO: deprecation, v0.3.0
        warnings.warn(
            "`DecodeOutputStreamer` is deprecated and will be " "removed in v0.3.0.",
            DeprecationWarning,
        )
    ...
    def _decode_fallback(self, value):
        ...
        tok = self.tokenizer.decode(
            value, skip_special_tokens=False, clean_up_tokenization_spaces=False
        )
        return tok + " "
...
class NoSpaceStreamer(DecodeOutputStreamer):
    def __init__(self, tokenizer, skip_prompt=True) -> None:
        ...
        # TODO: deprecation, v0.3.0
        warnings.warn(
            "`NoSpaceStreamer` is deprecated and will be " "removed in v0.3.0.",
            DeprecationWarning,
        )
        self.tokenizer = tokenizer
        self.skip_prompt = skip_prompt
...
def auto_dtype_of_deepspeed_config(ds_config):
    if ds_config.get("fp16") and not ds_config.get("bf16"):
        ...
        if ds_config.get("bf16").get("enabled") == "auto":
            ds_config["bf16"]["enabled"] = get_torch_device().is_bf16_supported()
        if (
            ds_config["fp16"]["enabled"] is True
