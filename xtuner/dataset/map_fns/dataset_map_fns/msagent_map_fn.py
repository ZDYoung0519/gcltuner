...
def replace_think(match):
    ...
    think_text = (
        match.group(3).replace("```JSON", "").replace("```", "").replace("\n", "")
    )
    think_json = json.loads(think_text)
    out_text += (
        f"Action:{think_json['api_name']}\n"
...
def replace_exec(match):
    out_text = ""
    exec_text = (
        match.group(2).replace("```JSON", "").replace("```", "").replace("\n", "")
    )
    ...
    for t in text:
        if t["from"] == "system":
            ...
            if think_cnt == 0:
                ...
                output = f"{output}\n"
            conversation.append(
                {"system": system_text, "input": input_text, "output": output}
            )
            system_text = ""
            input_text = ""
            if output_response is not None:
                try:
                    output_response, exec_cnt = re.subn(
                        exec_regex, replace_exec, output_response, flags=re.DOTALL
                    )
                    if "Final Answer:" in output_response:
                        ...
                        conversation.append(
                            {"system": output_response, "output": output_answer}
                        )
                except Exception:
                    pass
