...
def add_template_to_dataset(dataset, template_map_fn, map_num_proc):
    ...
    dataset = dataset.filter(
        lambda example: len(example["conversation"]) > 0, num_proc=map_num_proc
    )
    return dataset
...
def tokenize_dataset(
    ...
    dataset = dataset.map(
        ...
        remove_columns=list(dataset.column_names) if remove_unused_columns else None,
...
def pack_dataset(
    dataset, max_length, use_varlen_attn, shuffle_before_pack, map_num_proc
):
...
def process(
    ...
    if template_map_fn is not None:
        dataset = add_template_to_dataset(dataset, template_map_fn, map_num_proc)
