...
class MOSSSFTDataset(Dataset):
    def __init__(self, data_file, tokenizer, max_length=2048, bot_name=None):
        ...
        self.pre = len(self.tokenizer.encode("<|Results|>:", add_special_tokens=False))
        self.post = len(self.tokenizer.encode("<eor>\n", add_special_tokens=False))
        ...
        if os.path.exists(data_file) and os.path.exists(no_loss_spans_file):
            ...
            with open(self.src_data_file) as f:
                for line in tqdm(f):
                    ...
                    assert (
                        isinstance(instruction_ids, list) and len(instruction_ids) > 0
                    )
                    ...
                    try:
                        for i in range(num_turns):
                            ...
                            for key, value in cur_turn.items():
                                if self.bot_name is not None:
                                    value = value.replace("MOSS", self.bot_name)
                                ...
                                if key == "Tool Responses":
                                    ...
                                    cur_no_loss_spans.append(
                                        (
                                            len(input_ids + cur_turn_ids) + self.pre,
                                            len(input_ids + cur_turn_ids + cur_ids)
                                            - self.post,
                                        )
                                    )
                        ...
                        assert len(input_ids) > 0 and len(input_ids) <= self.max_length
