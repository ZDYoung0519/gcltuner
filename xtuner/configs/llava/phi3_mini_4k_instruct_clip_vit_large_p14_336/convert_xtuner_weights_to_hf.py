...
def convert_to_hf(text_model_id, vision_model_id, projector_weight, save_path):
    ...
    llm_state_dict = convert_state_dict_to_hf(
        llm_state_dict, KEYS_TO_MODIFY_MAPPING_LLM
    )
    vit_state_dict = convert_state_dict_to_hf(
        vit_state_dict, KEYS_TO_MODIFY_MAPPING_VIT
    )
    ...
    if added_token > 0:
        ...
        model.language_model.lm_head.weight.data[ori_vocab_size:] = torch.stack(
            tuple(
                dist.sample()
                for _ in range(
                    model.language_model.lm_head.weight.data[ori_vocab_size:].shape[0]
                )
            ),
            dim=0,
        )
...
def main():
    ...
    convert_to_hf(
        args.text_model_id, args.vision_model_id, args.projector_weight, args.save_path
    )
